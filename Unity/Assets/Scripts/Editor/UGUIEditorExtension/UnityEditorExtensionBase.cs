...
/// </summary>
public abstract class UnityEditorExtensionBase : UnityEditor.Editor
{
    ...
    static Assembly asm = Assembly.Load("UnityEditor.CoreModule");
	public UnityEditorExtensionBase (string editorTypeName)
	{
		var editorAssembly =asm;
		this.decoratedEditorType = editorAssembly.GetTypes().Where(t => t.Name == editorTypeName).FirstOrDefault();
		if (decoratedEditorType == null)
		{
			Debug.LogError("null type " + editorTypeName);
			decoratedEditorType = this.GetType();
			return;
		}
